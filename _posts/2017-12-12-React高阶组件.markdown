...
字符串类型的React元素（STRE）表示DOM节点，  
而功能类型React元素（FTRE）表示通过扩展React.Component创建的组件。  
For more about Elements and Components read this post.
有关元素和组件的更多信息请阅读[这篇文章](https://reactjs.org/blog/2015/12/18/react-components-elements-and-instances.html)。

FTRE将在Reacts[一致化处理](https://reactjs.org/docs/reconciliation.html)中解析为完整的STRE树（最终结果将始终为DOM元素）。

这很重要，这意味着**Inheritance Inversion高阶组件不保证解析完整子树**。
...
### Inheritance Inversion可以做什么？ ###

* 渲染劫持 Render Hijacking 
* 操纵状态 Manipulating state

#### 渲染劫持 Render Hijacking ####

之所以加Render Highjacking，是因为HOC控制了WrappedComponent的渲染输出，并且可以做各种各样的事。

有了渲染劫持你可以:

* 读取，添加，编辑，删除渲染输出的任何React Elements中的props
* 读取和修改渲染输出的React Elements树
* 有条件地显示元素树
* （类似Props代理的例子），把样式封装到元素树

注意，render是指WrappedComponent.render方法


> 你不能编辑或创建WrappedComponent实例的props，因为React Component不能编辑它收到的props，但是你可以改变从渲染方法输出的元素的道具。

就像前面讲的，II HOCs不保证解析完整子树，这就意味着对渲染劫持技术有一些限制。
