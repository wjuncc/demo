...
这里，W（WrappedComponent）指被封装的React.Component，E（指EnhancedComponent增强组件）指新的HOC，它返回React.Component。
...
The important part here is that the render method of the HOC returns a React Element of the type of the WrappedComponent.
这里的重要部分是HOC的render方法返回WrappedComponent类型的React元素。
We also pass through the props that the HOC receives, hence the name Props Proxy.
我们也通过了HOC收到的道具，所以叫做Props Proxy。
...
他们都创建了React元素来描述React应该在对账过程中呈现的内容，如果您想了解更多有关React元素与组件的信息，请参阅Dan Abramov的这篇文章，并参阅文档以了解更多有关对账流程的内容。
...
您可以阅读，添加，编辑和删除传递给WrappedComponent的道具。
...
小心删除或编辑重要的道具，你应该命名空间你的高阶道具不要打破WrappedComponent。

Example :  Adding new props.
示例:添加新的道具。
The apps current logged in user will be available in the WrappedComponent via this.props.user
当前登录用户的应用程序将通过this.props.user在WrappedComponent中可用


Accessing the instance via Refs
通过Refs访问实例

You can access this (the instance of the WrappedComponent) with a ref, but you will need a full initial normal render process of the WrappedComponent for the ref to be calculated, that means that you need to return the WrappedComponent element from the HOC render method
你可以用ref来访问这个（WrappedComponent的实例），但是需要一个完整的WrappedComponent的初始正常渲染过程来计算ref，这意味着你需要从HOC渲染方法返回WrappedComponent元素
, let React do its reconciliation process and just then you will have a ref to the WrappedComponent instance.
让React执行它的调整过程，然后你将有一个WrappedComponent实例的引用。
...
例子:在下面的例子中，我们探讨如何通过引用来访问WrappedComponent的实例方法和实例本身


When the WrappedComponent is rendered the ref callback will be executed, and then you will have a reference to the WrappedComponents instance.
当WrappedComponent呈现时，ref回调将被执行，然后你将有一个对WrappedComponents实例的引用。
This can be used for reading/adding instance props and to call instance methods.
这可以用于读取/添加实例道具和调用实例方法。
...
您可以通过向WrappedComponent提供道具和回调来抽象状态，这与智能组件处理哑元的方式非常类似。
See dumb and smart components for more information.
看到愚蠢和聪明的组件了解更多信息。
...
示例:在下面的状态抽象示例中，我们天真地抽象名称输入字段的值和onChange处理程序。
I say naively because this isnt very general but you got to see the point.
我天真地说，因为这不是很一般，但你必须看到这一点。
...
一些基本的用法可以通过定期的父组件来完成（见附录B），但是如前所述，您可以更灵活地使用HOC。
...
正如你所看到的，返回的HOC类（Enhancer）扩展了WrappedComponent。
It is called Inheritance Inversion because instead of the WrappedComponent extending some Enhancer class, it is passively extended by the Enhancer.
它被称为继承反转，因为不是扩展一些增强器类的WrappedComponent，而是由增强器被动扩展。
...
继承反转允许HOC通过这个访问WrappedComponent实例，这意味着它可以访问状态，道具，组件生命周期钩子和渲染方法。

I won't go into much detail about what you can do with lifecycle hooks because its not HOC specific but React specific.
我不会详细讨论你可以用生命周期钩子做什么，因为它不是HOC特定的，但是反应是特定的。
But note that with II you can create new lifecycle hooks for the WrappedComponent.
但请注意，使用II，您可以为WrappedComponent创建新的生命周期挂钩。
Remember to always call super.[lifecycleHook] so you don't break the WrappedComponent
记得总是调用super。[lifecycleHook]，这样就不会破坏WrappedComponent

Reconciliation process
调节过程

Before diving in lets summarize some theory.
在潜水之前让我们总结一些理论。

React Elements describe what is going to be rendered when React runs its reconciliation process.
React元素描述React运行其对帐过程时将要呈现的内容。
