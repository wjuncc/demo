...
如果你是React新手，建议去读[Reacts Docs](https://reactjs.org/docs/hello-world.html)。

高阶组件这个模式很精彩，好几个React库都证明其价值。

本文详解HOC是什么，能做什么，有什么限制，以及用法。

附录列举了我认为非HOC核心，但又需要了解的知识。

本文，尽数HOC之风流，如果你发现还有哪些内容没有讲到，请告知我，我来改。
...
2016年8月更新   
[日文版](http://postd.cc/react-higher-order-components-in-depth/)   
2017年1月更新   
[韩文版](https://www.vobour.com/book/view/XSSFQ5wBzsCLAbbo4)    
[中文版](https://zhuanlan.zhihu.com/p/24776678)   
感谢译者！

### 什么是高阶组件？ ###

就是组件套组件。俄罗斯套娃娃。
不是react官方，是react社区搞的，形式是函数，就是是工厂函数，下面用haskell这门语言写伪代码：
...
这里，  
W（WrappedComponent内组件）指被封装的组件，  
E（指EnhancedComponent外组件）指外组件，返回内组件。

注意，这里的“套”，有两种可能:

* Props Proxy（Props 代理） :   
外组件套内组件，外组件能操作内组件的Props，  
...
* 抽象state 和操作state（State abstraction and manipulation）

* 操作Props  （Props manipulation）

先讲HOC的实现方式，它决定HOC有哪些用法，又有哪些限制。

### HOC工厂方法的实现 ###

实现方式有两种： Props Proxy（PP）和Inheritance Inversion（II）。

虽然实现方式不同，但都能操作WrappedComponent。
...
删除，编辑重要属性时，要格外谨慎。应当加命名空间，避免外组件的props破坏内组件。
...
根据经验，使用渲染劫持，您将能够操纵WrappedComponent的render方法返回的元素树。
如果该元素树包含一个函数类型React组件，那么您将无法操作该子组件。 
（Reacts的一致化过程推迟了它实际渲染在屏幕上。）

实例1 :条件渲染。
这个HOC将完全渲染 WrappedComponent将渲染的内容，除非this.props.loggedIn不是true。 
（假设HOC收到登录的prop）
...
```
在这个例子中，如果WrappedComponent的render返回的有一个输入是在它的顶层元素，那么就把值改成“may the force be with you”。

可以在这里做各种各样的事，你可以遍历整个元素树并改变树中任何元素的props。
...
HOC可以读取，编辑和删除WrappedComponent实例的state，如果需要的话也可以添加更多的state。

请记住，这会搞乱WrappedComponent的 state，这会导致破坏。

大多数情况下，应该限制HOC读取或添加state，添加state时，应该放在命名空间，避免与WrappedComponents的state混淆。

示例:通过访问WrappedComponents的props and state进行调试
...
这个HOC用其他元素包装WrappedComponent，并显示WrappedComponents实例的 props 和 state。 
JSON.stringify 这个套路是[Ryan Florence](https://twitter.com/ryanflorence)和[Michael Jackson](https://twitter.com/mjackson)教我的。

在[这里](https://github.com/franleplant/react-hoc-examples/blob/master/ii_debug.js)看到调试器的完整实现。

#### 命名 ####

当用HOC封装组件时，会失去原来的WrappedComponents名字，这会影响开发和调试。

通常是在WrappedComponents组件名字前加上前缀，来自定义HOC名称。
以下代码来自React-Redux。
