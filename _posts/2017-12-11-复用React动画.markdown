...
复习一下，为了让过渡组内的组件更灵活，使其在不同是的阶段运行不同的动画，不在过渡组定义动画，而是在过渡组的子组件，在添加或删除子组件时，去调用相应的进入（enter）或离开（leave）的钩子函数。
...
最终[在线demo](https://codepen.io/cheapsteak/pen/RaGKOZ?editors=0010):
可以更进一步调试。
