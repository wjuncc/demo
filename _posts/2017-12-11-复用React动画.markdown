...
* 高阶组件，HOC，套，是一个东西，很重要，但是，本教程中用的PP，和[高阶教程]()的PP写法不同。用高阶教程，介绍的用法好像塞不进本教程的TweenMax。主要卡在render函数中，高阶教程中的函数传递可以传递子组件，但是，放在钩子函数就不知道该怎么写了。所以，下面的写法，还是用本篇叫教程的写法来做的。 
