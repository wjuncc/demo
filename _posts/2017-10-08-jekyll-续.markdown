...
了解如何将任何HTML网站转换成Jekyll模板将打开你的世界更多的选择Jekyll主题。 

不要在网上搜索Jekyll的主题，你可以从你网站的各种HTML模板中进行选择，快速地根据需要对HTML模板进行Jekyllize化，并用Jekyll构建输出。

尽管网站可以具有复杂的功能和控件，但是在本教程中保持简单。

什么是Jekyll网站？永久链接

首先，让我们从基础知识入手。","First, lets start with a grounding in the basics.",null,null,3],["将Jekyll网站剥离到一个非常基本的级别将有助于澄清在一个Jekyll网站发生的事情。","Stripping a Jekyll site down to an extremely basic level will help clarify what happens in a Jekyll site.",null,null,3],["如果你还没有安装jekyll gem，请安装它。

那么从一个基本的Jekyll站点组成三个文件_.._
...
default.html中
index.md
在名为my_jekyll_site的文件夹或任何最适合您的文件夹中手动创建这三个文件，并将default.html放在名为_layouts的文件夹中。

$ touch _config.yml index.md default.html
$ mkdir _layouts _and_ _and_ mv default.html _layouts
启动您最喜欢的编辑器，并按照以下方式填充default.html和index.md文件的内容_...
...
内容是用[Markdown]（https://www.learnxinyminutes.com/docs/markdown/）编写的。","Content is written in [Markdown](https _.._ //learnxinyminutes.com/docs/markdown/).",null,null,3],["纯文本格式允许您专注于您的**内容**。

\u003c！ - 
您可以在Markdown中使用HTML元素，例如comment元素，它们不会受到markdown解析器的影响。","You can use HTML elements in Markdown, such as the comment element, and they won't be affected by a markdown parser.",null,null,3],["但是，如果您在标记文件中创建HTML元素，则不能在该元素的内容中使用标记语法。
 - \u003e
