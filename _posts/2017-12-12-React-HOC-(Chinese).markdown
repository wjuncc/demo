...
（本段是废话）  
**翻译:**  
本文不适合React新手，[新手请移步](https://reactjs.org/docs/hello-world.html)。  
几个React库都表明“套”很管用。本文讲“套”是什么。能做什么，有什么限制，以及用法。   
附录，不是“套”，但又有点关系。   
本文全“套”，如有疏漏，不吝赐教。   
假设你有ES6知识。  
...
2016年8月更新：[日文版](http://postd.cc/react-higher-order-components-in-depth/)   
2017年1月更新：[韩文版](https://www.vobour.com/book/view/XSSFQ5wBzsCLAbbo4)，[中文版](https://zhuanlan.zhihu.com/p/24776678)   
...
**翻译:**  
就是组件套组件。俄罗斯套娃。
是react社区搞的，形式是工厂函数，用haskell语言写的伪代码：


	hocFactory:: W: React.Component => E: React.Component

这里，  
W（WrappedComponent内组件）指被套组件，  
E（EnhancedComponent外组件）返回内组件。

**题外话：** Haskell是什么？  Haskell（发音为/ˈhæskəl/）是一种纯函數程式語言，1990年在编程语言Miranda的基础上发展而来。

“套”，有两种:

* Props Proxy（Props 代理） : 
外组件套内组件，外组件操作内组件的Props，  
* Inheritance Inversion （继承颠倒）: 
外组件继承内组件 

## 套能做啥 ##

* 复用代码，提取逻辑，提取bootstrap

* 渲染劫持 （Render Hijacking）

* 提取state 和操作state（State abstraction and manipulation）

* 操作Props  （Props manipulation）

套的实现方式，决定了套有哪些用法，又有哪些限制。
...
* （PP）属性代理 Props Proxy
* （II）反向继承 Inheritance Inversion。

虽然性格不同，但志趣相投，都能操作内组件。
...
```
注意render方法，**返回** 传递进来的内组件。
返回的内容中，加上了外组件的props，所以叫**PP**。
...
这2种写法，都会创建React Element，描述了React在一致性过程中要render的内容。React Element与组件，参阅[Dan Abramov的这篇文章](https://reactjs.org/blog/2015/12/18/react-components-elements-and-instances.html)，还有官方文档[一致性处理](https://reactjs.org/docs/reconciliation.html)。

### PP能做啥 ###


* 操作属性（Manipulating props ）
* 通过Refs访问实例（Accessing the instance via Refs） 
* 抽象状态（Abstracting State）
* 用其他元素包装内组件（Wrapping the WrappedComponent with other elements）

#### 操作属性（Manipulating props） ####

可以读取，添加，编辑和删除传递给内组件的 props。

删/改重要属性时，要格外谨慎。应当加命名空间，避免外组件的props破坏内组件。

示例:添加props。  
下面代码，当前登录的用户，内组件通过this.props.user访问。
...
通过ref，访问this（内组件实例）。需要在内组件的render方法内执始化ref。也就是说，HOC的render方法必须返回内组件。
利用React的一致性处理，得到内组件实例的引用。

例子:通过[refs](https://reactjs.org/docs/refs-and-the-dom.html)，访问内组件的实例方法和实例本身
...
```
当内组件渲染时，会执行ref回调，就有了内组件实例的引用。
可以读/增实例props，和调用实例方法。

#### 提取状态（Abstracting State） ####

通过向内组件提供props和回调来提取状态，类似smart components处理dumb components。
参考[Presentational and Container Components](https://medium.com/@dan_abramov/smart-and-dumb-components-7ca2f9a7c7d0)。

实例:提取输入字段的值和onChange事件。  
注意，做项目不这么写，这只是个例子。
...
输入框将会自动成为[受控的input](https://reactjs.org/docs/forms.html)。

> 更多双向数据绑定HOC->[这个链接](https://github.com/franleplant/react-hoc-examples/blob/master/pp_state.js)

#### 用其他元素套内组件 ####

为了套样式，套布局，整合内组件与其他组件。
是的，简单需求，可以用定期的父组件来完成（见附录B），另外，灵活使用HOC，也能做到啊。
...
之所以叫反转继承，因为，不是内组件继承外组件，而是反过来（**inverse**），外组件继承内组件。   

继承反转使得HOC通过this访问内组件实例，也就是说**它可以访问state，props，生死钩和render方法**。

生死钩不是HOC特性，而是React特性。 
注意，内组件可创建生死钩。   
记得用super.[生死钩]，就不会破坏内组件。

#### 一致化处理（Reconciliation process） ####

先讲概念。

React元素对自身的描述，就决定了，在[一致化处理](https://reactjs.org/docs/reconciliation.html)时，它会渲染什么。 

React元素分两种类型:字串型和函数型。

字串型（简写STRE）表示DOM节点，  
函数型（简写FTRE）表示组件（继承React.Component）。  
[参考](https://reactjs.org/blog/2015/12/18/react-components-elements-and-instances.html)。

函数型[一致化处理](https://reactjs.org/docs/reconciliation.html)后解析为完整的字串型树（最终结果为DOM元素）。
这意味着**反转继承不保证虚拟Dom解析成完整子树**。
...
* 渲染劫持 Render Hijacking 
* 操作状态 Manipulating state

#### 渲染劫持 Render Hijacking ####

之所以叫渲染劫持，是因为HOC控制了内组件的渲染输出，可以做各种事：

* 读/增/写/删渲染输出的props
* 读/写渲染输出的React Elements树
* 有条件地显示元素树
* （类似Props代理），把样式封装到元素树
...
> 不能修改和创建内组件实例的props，因为React组件不能修改父组件的props，但可以修改渲染输出的元素的props。

因为反转继承不保证解析完整子树，所以渲染劫持有限制。

经验告诉我，渲染劫持，能操作内组件的render方法返回的元素树。
如果该元素树包含函数型组件，将无法操作该子组件。 
（Reacts的一致化过程推迟了它实际渲染在屏幕上。）

实例1 :条件渲染。
当this.props.loggedIn等于true，将完全渲染内组件所渲染的内容。 
（假设HOC收到登录的prop）
...
实例2 :修改由render返回的React Elements树。
...
```
如果内组件的render返回输入是在它的顶层元素，那么就把值改成“may the force be with you”。

可以在做各种事，例如遍历整个元素树并改变树中任何元素的props。

这正是[Radium](http://stack.formidable.com/radium/) 的做法（后面的案件讲了Radium）。

注意:Props Proxy不能做渲染劫持。

虽然可以用内组件.prototype.render访问渲染方法，但要模拟内组件实例和Props，还要自己处理生死，而不是React。
所以说不值得。做渲染劫持，用反转继承，而不是Props代理。
请记住，React在内部处理组件实例，处理实例的唯一方法是通过this或refs 


#### 操作state ####

HOC可以读写删（也可以增）内组件实例的state。
这会搞乱内组件。

大多数情况下，应限制HOC读取或添加state，增加state，应放在命名空间，避免与内组件的state混淆。

示例:访问内组件的props and state进行调试
...
这个HOC用其他元素包装内组件，并显示内组件实例的 props 和 state。 
JSON.stringify是[Ryan Florence](https://twitter.com/ryanflorence)和[Michael Jackson](https://twitter.com/mjackson)教我的。

[完整实现demo](https://github.com/franleplant/react-hoc-examples/blob/master/ii_debug.js)。

#### 命名 ####

用HOC套组件，内组件会失去原名，影响开发调试。
...
它提供的功能之一是连接，它处理所有bootstrap需要监听存储和之后的清理。
这是通过Props代理实现来实现的。


如果你用过纯的Flux，那么会知道，任何连接到一个或多个store的React组件需要大量的 bootstrapping 来添加和删除stores 监听器，并选择他们需要的state部分。
所以React-Redux的实现是非常好的，因为它抽象了这些。
基本上，你不需要自己写了！

#### [Radium](http://formidable.com/open-source/radium/) ####

Radium是样式库,[内联样式](https://developer.mozilla.org/en-US/docs/Web/CSS/Pseudo-classes)CSS伪选择器。
内联样式有什么好处我不想多说，但是很多人这样做，Radium库更快的解决了问题。
内联样式参阅[Vjeux](https://medium.com/@vjeux)的[演示](https://speakerdeck.com/vjeux/react-css-in-js)
...
现在我们将回顾一下家长组件可以做什么和不可以做什么与HOCs相比，还有一些重要的细节：
...
#### 结束语 ####
我希望在阅读这篇文章之后，你对React HOC有更多的了解。
他表现强劲，在不同库里被证明是相当不错的。

React带来了很多创新，像Radium，React-Redux，React-Router等项目就是很好的证明。

想要联系我，请关注我twitter@franleplant。

这个[仓库](https://github.com/franleplant/react-hoc-examples)写了一些代码，试验了一些在文章中出现的模式。

### 感谢 ###
感谢React-Redux，Radium，[Sebastian Markb](https://twitter.com/sebmarkbage)的[仓库](https://gist.github.com/sebmarkbage/ef0bf1f338a7182b6775)和我自己的实验。
...
2016年5月28日    
好文章！   
but does much of the property transformation that you wrote about and more (for other folks that might not be familiar with it).
我是一个重组的爱好者，就是你提到的命名，但是你写的很多属性转换和更多的（对于其他人可能不熟悉它）。
