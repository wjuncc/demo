...
您已经使用{ {page.title} }标签配置了页面标题。   
但有更多的标题标签来填充。  
页面也有一个标题标签，出现在浏览器选项卡或窗口中。  
通常，您将网页标题放在网站标题后面。  

在你的default.html布局中，找到头标下的标题标签：  
...
标题：   
ACME网站   
您在_config.yml文件中添加的任何属性均可通过站点名称空间访问。    
同样，页面前面的任何属性都可以通过页面名称空间访问。   
在站点或页面之后使用点符号来访问值。  

用Ctrl + C停止你的Jekyll服务器并重新启动它。   
验证标题标签正确填充。   

每当你修改你的配置文件时，你必须重新启动Jekyll才能使更改生效。   
当您修改其他文件时，Jekyll在重建时会自动获取更改。   
...
它通常显示主页上的帖子列表。  
首先，让我们创建一些帖子，以便我们展示一些东西。  

在_posts文件夹中添加标准YYYY-MM-DD-title.md文章格式的文章：
...
一些示例内容： 
现在让我们创建一个布局来显示帖子。  
在_layouts中创建一个名为home.html的新文件，并添加以下逻辑_.._
...
布局_.._家
---
在这种情况下，blog.md的内容将被推送到家庭布局中的{ {content} }标签。  
然后，家庭布局将被推入默认布局的{ {content} }标签。   

### 布局如何工作固定链接 ###

当布局指定另一个布局时，这意味着第一个布局的内容将填充到第二个布局的{ {content} }标记中。   
每个布局适合其指定的另一个布局。
...
在此示例中，指定布局_.._文档的Markdown文档document.md中的内容被推送到布局文件docs.html的{ {content} }标签中。   
因为文档布局本身指定了布局_.._页面，所以来自docs.html的内容被压入布局文件page.html中的{ {content} }标签。   
最后，因为页面布局指定了布局_.._默认值，所以page.html中的内容被推送到布局文件default.html的{ {content} }标签中。

你不需要多个布局。  
你可以使用一个默认。   
您可以选择如何设计您的网站。  
一般来说，为页面定义一个布局，为帖子定义另一个布局，但是这两个布局要继承默认模板（通常定义站点的顶部和底部部分）。

在您的浏览器中，转到blog.html并查看帖子列表。  
请注意，您不必使用此处所述的方法。   
您可以简单地将for循环添加到任何页面，如index.md，以显示这些帖子。  
...
顺便说一句，让我们暂停一下，看看for循环逻辑更紧密一点。  
Liquid中的循环是最常用的Liquid标签之一。   
For循环让你遍历你的Jekyll网站的内容，并建立一个结果。   
for循环也具有某些可用的属性（如第一次或最后一次迭代），也是基于循环中的循环位置。

我们只是抓了你可以用循环来检索帖子的表面。  
例如，如果您想要显示来自特定类别的帖子，可以通过为您的帖子前置事项添加类别属性，然后查看这些类别。  
此外，您可以通过添加限制属性来限制结果的数量。  
下面是一个例子_.._
...
现在你已经配置了帖子，让我们配置页面导航。
大多数网站在侧边栏或标题区域都有一些导航。

在本教程中，假设您已经获得了一个简单的想要生成的页面列表。
如果您只有少数几页，则可以使用for循环遍历site.pages对象来列出它们，然后通过front matter属性对它们进行排序。

确定出现页面列表的代码部分。
通常这是一个带有各种子元素的\u003cul\u003e元素。
将代码替换为以下_.._
...
您也可以迭代您在单独的数据文件中维护的页面列表。
如果您有很多页面，或者您有关于要存储的页面的其他属性，这可能会更合适。

要以这种方式管理页面链接，请在您的Jekyll项目中创建一个名为_data的文件夹。
在此文件夹中，创建一个名为例如_.._
...
如果你以前从未写过任何YAML，那么你很快就会熟悉它。
看看你可以用YAML做什么。

您可以根据需要为此数据文件中的每个项目存储其他属性。
按照您希望显示的顺序排列列表项目。
...
让我们假设你的default.html文件很庞大，很难使用。
您可以通过将一些HTML代码放入包含文件中来分解您的布局。

在根目录中添加名为_includes的文件夹。
在该文件夹中，在那里添加一个名为sidebar.html的文件。
...
你可以打破你的主题的其他元素，如你的页眉或页脚。
然后，您可以将这些常用元素应用于其他布局文件。
这样你就不会有重复的代码。
...
您的Jekyll站点需要RSS提要。
下面是基本的RSS提要语法。
要在Jekyll中创建RSS文件，请在根目录下创建一个名为feed.xml的文件，并添加以下代码_.._
...
此代码使用for循环来查看您最近的20个帖子。
来自帖子的内容被转义并被截断为使用液体过滤器的最后400个字符。

在您的default.html布局中，查找头中RSS或Atom提要的引用，并将其替换为对刚刚创建的文件的引用。
例如 _.._

\u003clink rel =“alternate”type =“application / rss + xml”href =“{ {site.url} } / feed.xml”title =“{ {site.title} }”\u003e
您还可以通过添加名为jekyll-feed的gem来自动生成帖子Feed。
这个宝石也将在GitHub页面上工作。

11.添加一个sitemapPermalink

最后，添加一个网站地图。
在根目录下创建一个sitemap.xml文件并添加这个代码_.._
...
您也可以通过添加名为jekyll-sitemap的gem来自动生成您的站点地图。
这个宝石也将在GitHub页面上工作。

12.添加外部服务Permalink

对于您可能需要的其他服务（如联系表单，搜索，评论等），请查找第三方服务。
例如，您可以使用以下_.._
...
在您为这些服务集成代码时，请注意，如果您的Jekyll站点中的页面没有前置事项标记，那么Jekyll将不会处理该页面中的任何内容。
当您建立您的网站时，页面将被传递到_site文件夹。
