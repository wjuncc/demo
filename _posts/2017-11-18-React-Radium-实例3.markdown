...
```
For the `button` tags has just only one `style` attribute to store the inline style code, and it will be translate finally to inline style by react autoly.There is no no choice but one target put all information into one either simple style or complex radium define. In other words, We cant create new attribute, it will muss the tag-attribute nonsense .
