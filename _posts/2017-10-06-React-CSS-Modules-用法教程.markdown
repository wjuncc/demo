...
Create winform interface is an easy task and  happy experience with coding in powershell, which is  harder created in C# with visual studio in developing a same function. The code will be very clear without complex communication object. Also we will catch the error message in terminal easily instead of avoiding the complex debug system in vs.
