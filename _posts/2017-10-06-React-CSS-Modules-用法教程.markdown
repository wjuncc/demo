...
Create winform interface is a happy and easy task in coding with powershell, which is much more easily than created in C# with visual studio in developing a same function. The code will be very clear without complex communication object. Also we will catch the error message in terminal easily instead of avoiding the complex debug system in vs.
