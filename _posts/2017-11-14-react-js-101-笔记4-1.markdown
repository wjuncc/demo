...
本课是用 [官网范例](https://facebook.github.io/react/index.html)一共4个例子。分别是：

*  Props - 父组件怎样传递值给子组件
*  State - 在状态值变化的时候，如何刷新UI
*  事件 - 一个最简化的todolist
*  表单处理。 

4个例子都很简单，只是蜻蜓点水，点到为止。
代码敲一遍，OR [jsbin](http://jsbin.com/) ！  

## 例1 Props

文件位置：E:\n\learn\react\101\Ch04\demo  
访问地址：http://127.0.0.1/learn/react/101/Ch04/demo/
为了方便没有用webpacket, 用的是。   
index.html要加上browser.min.js引用, 
```html
<script src="js/browser.min.js"></script>
```
app.js引用的地方要加上 type="text/babel" 属性。
```html
...
注意，直接访问
file:///E:/learn/react/101/Ch04/demo/index.html会报错：
![1](https://i.imgur.com/Ur3TMHC.png)
需要apache等各种本地http服务器，访问：
http://127.0.0.1/learn/react/101/Ch04/demo/
![1](https://i.imgur.com/zWuykPV.png) 

[React ES6 父类super()](http://cheng.logdown.com/posts/2016/03/26/683329) 。

使用 Functional Component 写法：  
文件目录：E:\n\learn\react\101\Ch04\demo2 
```javascript
...
ReactDOM.render(<HelloMessage name="Mark456" />, document.getElementById('app'));
```
![3](https://i.imgur.com/g5Kgs0L.png)
在 jsbin 上的范例：
...
路径：E:\n\learn\react\101\Ch04\demo3  
访问地址：http://127.0.0.1/learn/react/101/Ch04/demo3/
官网： A Stateful Component 范例。   
用 `this.state` 存取 state。  
一旦`setState()` 方法更新 state就调用重绘方法 `render()` ，  
每 1000 毫秒（等于1秒）+1。
只改app.js： 
```javascript
...
```
![3](https://i.imgur.com/ZlwtMo8.png)

关于 Javascript this 用法可以参考 [Javascript：this用法整理](https://software.intel.com/zh-cn/blogs/2013/10/09/javascript-this)。

## 例3 事件

路径：E:\n\learn\react\101\Ch04\demo4   
访问地址：http://127.0.0.1/learn/react/101/Ch04/demo4/
React 官网 An Application 例子。  
只改app.js：
...
```
![4](https://i.imgur.com/F0IeEnV.png)
常用有`onChange` `onSubmit` `onClick`等。[官网 Event System](https://facebook.github.io/react/docs/events.html)。

## 例4 Refs 与表单
路径：E:\n\learn\react\101\Ch04\demo5   
访问地址：http://127.0.0.1/learn/react/101/Ch04/demo5/
官网范例 A Component Using External Plugins。
![4](https://i.imgur.com/nQyjtmI.png)
本例用 `remarkable` 结合 `ref` 属性取出 DOM Value 值（另外比较常用的作法是使用 `onChange` 事件处理方式处理表单内容）。
除了引入 `react` 、 `react-dom` 还要用 `CDN` 方式引入 `remarkable` 这个 `Markdown` 语法 parser 套件，记得如果没有使用 Webpack 或是 browserify + babelify 等工具需要引入 `babel-standalone` 浏览器解析 ES6 语法并于引入 script 加上 type="text/babel"
HTML：
